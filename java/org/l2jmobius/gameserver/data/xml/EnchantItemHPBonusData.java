...
public class EnchantItemHPBonusData implements IXmlReader {
	...
	private static final float FULL_ARMOR_MODIFIER = 1.5f; // Enchant HP bonus multiplier (hardcoded).
			...
			for (Integer itemId : armorIds) {
				...
				if ((item != null) && (item.getCrystalType() != CrystalType.NONE)) {
					switch (item.getBodyPart()) {
						...
						case L_HAND: {
							item.attach(
									new FuncTemplate(null, null, StatFunction.ENCHANTHP.getName(), -1, Stat.MAX_HP, 0));
							break;
						}
						default: {
							break;
