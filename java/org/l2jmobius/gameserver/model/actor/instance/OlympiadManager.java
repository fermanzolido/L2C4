			...
			switch (val) {
				...
				case 2: {
					...
					if (OlympiadConfig.OLYMPIAD_REG_DISPLAY > 0) {
						html.replace("%listClassed%",
								classed < OlympiadConfig.OLYMPIAD_REG_DISPLAY ? FEWER_THAN : MORE_THAN);
						html.replace("%listNonClassedTeam%", FEWER_THAN);
						html.replace("%listNonClassed%",
								nonClassed < OlympiadConfig.OLYMPIAD_REG_DISPLAY ? FEWER_THAN : MORE_THAN);
					} else {
				...
				case 3: {
					...
					final int elo = Olympiad.getNobleStats(player.getObjectId()).getInt(Olympiad.ELO);
					html.setFile(player, Olympiad.OLYMPIAD_HTML_PATH + "noble_points1.htm");
					html.replace("%points%", String.valueOf(points));
					html.replace("%elo%", String.valueOf(elo));
					html.replace("%division%", Olympiad.getDivision(elo));
					html.replace("%objectId%", String.valueOf(getObjectId()));
					player.sendPacket(html);
					break;
				}
				...
				case 11: {
					html.setFile(player, Olympiad.OLYMPIAD_HTML_PATH + "season_history.htm");
					final StringBuilder sb = new StringBuilder();
					try (Connection con = DatabaseFactory.getConnection();
							PreparedStatement statement = con.prepareStatement(
									"SELECT * FROM olympiad_seasons_history ORDER BY season_id DESC, elo DESC LIMIT 20")) {
						try (ResultSet rset = statement.executeQuery()) {
							while (rset.next()) {
								sb.append("Season ").append(rset.getInt("season_id")).append(": ");
								sb.append(rset.getString("char_name")).append(" (").append(rset.getInt("elo"))
										.append(") - ").append(rset.getString("division")).append("<br>");
							}
						}
					} catch (SQLException e) {
						_logOlymp.warning("Olympiad System: Could not load season history: " + e.getMessage());
					}

					if (sb.length() == 0) {
						sb.append("No history available yet.");
					}

					html.replace("%history%", sb.toString());
					html.replace("%current_season%", String.valueOf(Olympiad.getInstance().getCurrentSeason()));
					html.replace("%objectId%", String.valueOf(getObjectId()));
					player.sendPacket(html);
					break;
				}
				case 12: {
					Olympiad.getInstance().claimSeasonReward(player);
					break;
				}
				default: {
