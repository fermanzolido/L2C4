...
public class SevenSignsFestival {
	 ...
	 * All times are relative to the ELAPSED time (in ms) since a festival
	 * begins.<br>
	 ...
	 * length.<br>
	 * This allows ample time for players to sign-up to participate in the
	 * festival.<br>
	 * The intermission is the time between the festival participants being moved to
	 * the "arenas" and the spawning of the first set of mobs.<br>
	 * The monster swarm time is the time before the monsters swarm to the center of
	 * the arena, after they are spawned.<br>
	 * The chest spawn time is for when the bonus festival chests spawn, usually
	 * towards the end of the festival.
	 */
	public static final long FESTIVAL_SIGNUP_TIME = FeatureConfig.ALT_FESTIVAL_CYCLE_LENGTH
			- FeatureConfig.ALT_FESTIVAL_LENGTH - 60000;
	 ...
	 * and should be treated as such. Data is initially accessed by the related
	 * Seven Signs cycle, with _signsCycle representing data for the current round
	 * of Festivals. The actual table data is stored as a series of StatSet
	 * constructs. These are accessed by the use of an offset based on the number of
	...
	/**
	 * Primarily used to terminate the Festival Manager, when the Seven Signs period
	 * changes.
	 * 
	...
	protected void startFestivalManager() {
		...
		_managerScheduledTask = ThreadPool.scheduleAtFixedRate(_managerInstance,
				FeatureConfig.ALT_FESTIVAL_MANAGER_START, FeatureConfig.ALT_FESTIVAL_CYCLE_LENGTH);

		LOGGER.info("SevenSignsFestival: The first Festival of Darkness cycle begins in "
				+ (FeatureConfig.ALT_FESTIVAL_MANAGER_START / 60000) + " minute(s).");
	}
	...
	protected void restoreFestivalData() {
		try (Connection con = DatabaseFactory.getConnection();
				...
				ResultSet rs = s.executeQuery(
						"SELECT festivalId, cabal, cycle, date, score, members FROM seven_signs_festival")) {
			while (rs.next()) {
	...
	/**
	 * Stores current festival data, basic settings to the properties file and past
	 * high score data to the database. If updateSettings = true, then all Seven
	...
	/**
	 * Returns the current festival ID and oracle ID that the specified player is
	 * in, but will return the default of {-1, -1} if the player is not found as a
	 * participant.
	 * 
	 * @param player
	...
	/**
	 * Returns a stats set containing the highest ever recorded score data for the
	 * specified festival.
	 * 
	 * @param festivalId
	...
	public boolean setFinalScore(Player player, int oracle, int festivalId, long offeringScore) {
		...
		// Check if this is the highest score for this level range so far for the
		// player's cabal.
		if (offeringScore > thisCabalHighScore) {
			...
			// Only add the score to the cabal's overall if it's higher than the other
			// cabal's score.
	 ...
	 * where the player was in the winning party, if the winning party's cabal won
	 * the event. The accumulated bonus is then updated, with the player's share
	 * deducted.
	 * 
	 * @param player
	...
	/**
	 * Basically a wrapper-call to signal to increase the challenge of the specified
	 * festival.
	 * 
	 * @param oracle
	 * @param festivalId
	...
	/**
	 * The FestivalManager class is the main runner of all the festivals. It is used
	 * for easier integration and management of all running festivals.
	 * 
	 * @author Tempy
	 */
	private class FestivalManager implements Runnable {
		...
		public synchronized void run() {
			try {
				...
				// Clear past participants, they can no longer register their score if not done
				// so already.
				_dawnPreviousParticipants.clear();
				_duskPreviousParticipants.clear();
				...
				/* PROPOGATION */
				// Sequentially set all festivals to begin, spawn the Festival Witch and notify
				// participants.
				for (DarknessFestival festivalInst : _festivalInstances.values()) {
				...
				for (DarknessFestival festivalInst : _festivalInstances.values()) {
					...
					final long end = (FeatureConfig.ALT_FESTIVAL_LENGTH - FeatureConfig.ALT_FESTIVAL_SECOND_SPAWN)
							/ 60000;
					if (end == 2) {
						...
						festivalInst.sendMessageToParticipants(
								"The Festival of Darkness will end in " + end + " minute(s).");
					}
				}

				elapsedTime += FeatureConfig.ALT_FESTIVAL_SECOND_SPAWN - FeatureConfig.ALT_FESTIVAL_FIRST_SWARM;

				// After another short time period, again move all idle spawns to the center of
				// the arena.
				try {
				...
				for (DarknessFestival festivalInst : _festivalInstances.values()) {
					...
					festivalInst.sendMessageToParticipants(
							"The chests have spawned! Be quick, the festival will end soon."); // NpcString needs
																								// verification.
				}
				...
				// Participants can no longer opt to increase the challenge, as the festival
				// will soon close.
		...
		public DarknessFestival getFestivalInstance(int oracle, int festivalIdValue) {
			...
			/*
			 * Compute the offset if a Dusk instance is required. ID: 0 1 2 3 4 Dusk
			 * 1:1011121314 Dawn 2:2021222324
			 */
	...
	/**
	 * Each running festival is represented by an DarknessFestival class. It
	 * contains all the spawn information and data for the running festival. All
	 * festivals are managed by the FestivalManager class, which must be initialized
	 * first.
	 * 
	 * @author Tempy
	 */
	private class DarknessFestival {
		...
		protected void festivalInit() {
			...
			if ((_participants != null) && !_participants.isEmpty()) {
				try {
					for (int participantObjId : _participants) {
						...
						_originalLocations.put(participantObjId, new FestivalSpawn(participant.getX(),
								participant.getY(), participant.getZ(), participant.getHeading()));
			...
			try {
				...
				LOGGER.log(Level.WARNING, "SevenSignsFestival: Error while spawning Festival Witch ID "
		...
		protected void moveMonstersToCenter() {
			for (FestivalMonster festivalMob : _npcInsts) {
				 ...
				 * maximum allowed offset are generated to prevent the mobs from all moving to
				...
				festivalMob.getAI().setIntention(Intention.MOVE_TO,
						new Location(x, y, _startLocation._z, Rnd.get(65536)));
			}
		}
		...
		protected void spawnFestivalMonsters(int respawnDelay, int spawnType) {
			...
			switch (spawnType) {
				case 0:
				case 1: {
					npcSpawns = (_cabal == SevenSigns.CABAL_DAWN) ? FESTIVAL_DAWN_PRIMARY_SPAWNS[_levelRange]
							: FESTIVAL_DUSK_PRIMARY_SPAWNS[_levelRange];
					break;
				}
				case 2: {
					npcSpawns = (_cabal == SevenSigns.CABAL_DAWN) ? FESTIVAL_DAWN_SECONDARY_SPAWNS[_levelRange]
							: FESTIVAL_DUSK_SECONDARY_SPAWNS[_levelRange];
					break;
				}
				case 3: {
					npcSpawns = (_cabal == SevenSigns.CABAL_DAWN) ? FESTIVAL_DAWN_CHEST_SPAWNS[_levelRange]
			...
			for (int[] spawn : npcSpawns) {
				...
				try {
					...
					LOGGER.log(Level.WARNING, "SevenSignsFestival: Error while spawning NPC ID " + currSpawn._npcId
							+ ": " + e.getMessage(), e);
				}
			}
		}
