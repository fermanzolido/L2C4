...
public class Olympiad extends ListenersContainer {
	...
	private static final String OLYMPIAD_LOAD_DATA = "SELECT current_cycle, period, olympiad_end, validation_end, next_weekly_change, current_season FROM olympiad_data WHERE id = 0";
	private static final String OLYMPIAD_SAVE_DATA = "INSERT INTO olympiad_data (id, current_cycle, period, olympiad_end, validation_end, next_weekly_change, current_season) VALUES (0,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE current_cycle=?, period=?, olympiad_end=?, validation_end=?, next_weekly_change=?, current_season=?";
	...
	private static final String OLYMPIAD_INSERT_HISTORY = "INSERT INTO olympiad_seasons_history (season_id, charId, char_name, class_id, olympiad_points, elo, division) VALUES (?,?,?,?,?,?,?)";
	...
	private void load() {
		...
		try (Connection con = DatabaseFactory.getConnection();
			...
			while (rset.next()) {
				...
				_currentSeason = rset.getInt("current_season");
			}
			loaded = true;
		} catch (Exception e) {
		...
		try (
			...
			while (rset.next()) {
				...
				statData.set(ELO, rset.getInt(ELO));
		...
		LOGGER.info("Olympiad System: " + numDays + " days, " + numHours + " hours and " + numMins
				+ " mins until period ends.");
	...
	protected class ValidationEndTask implements Runnable {
		...
		public void run() {
			...
			if (_currentCycle >= OlympiadConfig.OLYMPIAD_SEASON_DURATION) {
				performSeasonReset();
			} else {
				_currentCycle++;
			}
	...
	public boolean registerNoble(Player noble, boolean classBased) {
		/*
		 * if (_compStarted) {
		 * noble.sendMessage("Cannot Register whilst competition is under way"); return
		 * false; }
		 */
		...
		if (!NOBLES.containsKey(noble.getObjectId())) {
			...
			statDat.set(ELO, OlympiadConfig.OLYMPIAD_ELO_INITIAL_VALUE);
	...
	public static int getNobleElo(int charId) {
		final StatSet set = NOBLES.get(charId);
		return set != null ? set.getInt(ELO) : OlympiadConfig.OLYMPIAD_ELO_INITIAL_VALUE;
	}
	...
	private void updateCompStatus() {
		...
		final int numHours = (int) Math.floor(countDown % 24);
		final int numDays = (int) Math.floor((countDown - numHours) / 24);
		LOGGER.info("Olympiad System: Competition Period Starts in " + numDays + " days, " + numHours + " hours and "
				+ numMins + " mins.");
		...
		_scheduledCompStart = ThreadPool.schedule(() -> {
			...
			if (regEnd > 0) {
				ThreadPool.schedule(
						() -> Broadcast.toAllOnlinePlayers("The Grand Olympiad registration period has ended."),
						regEnd);
			}
	...
	protected synchronized void saveNobleData() {
		...
		try (Connection con = DatabaseFactory.getConnection()) {
			...
			for (Entry<Integer, StatSet> entry : nobleList) {
				...
				final int elo = nobleInfo.getInt(ELO);
				final boolean toSave = nobleInfo.getBoolean("to_save");

				try (PreparedStatement statement = con
						.prepareStatement(toSave ? OLYMPIAD_SAVE_NOBLES : OLYMPIAD_UPDATE_NOBLES)) {
					if (toSave) {
	...
	public List<String> getClassLeaderBoard(int classId) {
		...
		final String query = OlympiadConfig.OLYMPIAD_SHOW_MONTHLY_WINNERS
	...
	public int getLastNobleOlympiadPoints(int objId) {
		...
		try (Connection con = DatabaseFactory.getConnection();
				PreparedStatement ps = con
						.prepareStatement("SELECT olympiad_points FROM olympiad_nobles_eom WHERE charId = ?")) {
			ps.setInt(1, objId);
	...
	private static void handleSpectatorEnter(int gameId, Player player) {
		...
		if (game != null) {
			if ((game._playerOne == null) || !game._playerOne.isInOlympiadMode()
					|| !game._playerOne.isOlympiadStart()) {
				return;
			}

			if ((game._playerTwo == null) || !game._playerTwo.isInOlympiadMode()
	...
	private void performSeasonReset() {
		final int oldSeason = _currentSeason;
		_currentSeason++;
		...
		try (Connection con = DatabaseFactory.getConnection();
			...
			for (Entry<Integer, StatSet> entry : NOBLES.entrySet()) {
				...
				if (elo > initialElo) {
					final int newElo = initialElo
							+ (int) ((elo - initialElo) * OlympiadConfig.OLYMPIAD_SEASON_SOFT_RESET_MULTIPLIER);
					set.set(ELO, newElo);
				} else {
					set.set(ELO, initialElo);
				}

				// Reward Delivery (Inventory if online, otherwise we'd need a different system
	...
	private void giveSeasonReward(int charId, String division, List<StatSet> offlineRewards) {
		...
		switch (division) {
			case DIV_BRONZE:
				rewardId = OlympiadConfig.OLYMPIAD_SEASON_BRONZE_REWARD_ID;
				rewardCount = OlympiadConfig.OLYMPIAD_SEASON_BRONZE_REWARD_COUNT;
				break;
			case DIV_SILVER:
				rewardId = OlympiadConfig.OLYMPIAD_SEASON_SILVER_REWARD_ID;
				rewardCount = OlympiadConfig.OLYMPIAD_SEASON_SILVER_REWARD_COUNT;
				break;
			case DIV_GOLD:
				rewardId = OlympiadConfig.OLYMPIAD_SEASON_GOLD_REWARD_ID;
				rewardCount = OlympiadConfig.OLYMPIAD_SEASON_GOLD_REWARD_COUNT;
				break;
			case DIV_PLATINUM:
				rewardId = OlympiadConfig.OLYMPIAD_SEASON_PLATINUM_REWARD_ID;
				rewardCount = OlympiadConfig.OLYMPIAD_SEASON_PLATINUM_REWARD_COUNT;
				break;
			case DIV_DIAMOND:
				rewardId = OlympiadConfig.OLYMPIAD_SEASON_DIAMOND_REWARD_ID;
				rewardCount = OlympiadConfig.OLYMPIAD_SEASON_DIAMOND_REWARD_COUNT;
				break;
		}
	...
	public void claimSeasonReward(Player player) {
		if (player == null) {
			return;
		}

		boolean found = false;
		// Check for all past seasonsRewards (up to current)
		for (int i = 1; i <= _currentSeason; i++) {
			final String varName = "SEASON_REWARD_" + i;
			final String val = player.getVariables().getString(varName, null);
			if (val != null) {
		...
		if (!found) {
			player.sendMessage("You have no pending Olympiad season rewards.");
		}
	}
}
