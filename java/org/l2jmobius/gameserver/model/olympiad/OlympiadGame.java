...
class OlympiadGame {
	...
	protected boolean portPlayersToArena() {
		...
		try {
			...
			// Advanced Elo: Divisions & Titles
			if (OlympiadConfig.OLYMPIAD_ELO_ENABLE_TITLES) {
				final int elo1 = Olympiad.getNobleStats(_playerOneID).getInt(Olympiad.ELO);
				final int elo2 = Olympiad.getNobleStats(_playerTwoID).getInt(Olympiad.ELO);

				_playerOne.getAppearance().setTitleColor(Olympiad.getDivisionColor(elo1));
				_playerOne.setTitle("[" + Olympiad.getDivision(elo1) + "]");
				_playerOne.broadcastTitleInfo();

				_playerTwo.getAppearance().setTitleColor(Olympiad.getDivisionColor(elo2));
				_playerTwo.setTitle("[" + Olympiad.getDivision(elo2) + "]");
				_playerTwo.broadcastTitleInfo();
			}
	...
	protected boolean haveWinner() {
		if (_aborted || (_playerOne == null) || (_playerTwo == null) || _playerOneDisconnected
	...
	protected void validateWinner() {
		...
		final int pointDiff = Math.min(Math.min(playerOnePoints, playerTwoPoints) / _div,
				OlympiadConfig.OLYMPIAD_MAX_POINTS);

		// Elo Calculation
		final int eloOne = playerOneStat.getInt(Olympiad.ELO);
		final int eloTwo = playerTwoStat.getInt(Olympiad.ELO);
		final double expectedOne = 1.0 / (1.0 + Math.pow(10, (eloTwo - eloOne) / 400.0));
		final double expectedTwo = 1.0 - expectedOne;

		// Advanced Elo: Anti-Feeding (Elo gap check)
		final boolean skipEloUpdate = Math.abs(eloOne - eloTwo) > OlympiadConfig.OLYMPIAD_ELO_MAX_GAP;

		// Advanced Elo: Dynamic Rewards scaling
		final int dynamicReward = (int) (_gpreward * ((eloOne + eloTwo) / 2000.0)
				* OlympiadConfig.OLYMPIAD_ELO_REWARD_MULTIPLIER);
		final int finalReward = Math.max(_gpreward, dynamicReward); // Never less than base reward
		...
		if (_pOneCrash || _pTwoCrash) {
			if (_pOneCrash && !_pTwoCrash) {
				try {
					...
					// Notify to scripts
					if (EventDispatcher.getInstance().hasListener(EventType.ON_OLYMPIAD_MATCH_RESULT,
				...
				try {
					...
					if (EventDispatcher.getInstance().hasListener(EventType.ON_OLYMPIAD_MATCH_RESULT,
							Olympiad.getInstance())) {
						EventDispatcher.getInstance().notifyEventAsync(
								new OnOlympiadMatchResult(null, _playerOne, _type), Olympiad.getInstance());
						EventDispatcher.getInstance().notifyEventAsync(
								new OnOlympiadMatchResult(null, _playerTwo, _type), Olympiad.getInstance());
					}
				} catch (Exception e) {
					LOGGER.log(Level.WARNING, "Exception on validateWinnder(): " + e.getMessage(), e);
				}
		...
		if ((_playerOne == null) && (_playerTwo == null)) {
			...
			playerTwoStat.set(COMP_LOST, playerTwoLost + 1);
			playerOneStat.set(Olympiad.ELO,
					eloOne + (skipEloUpdate ? 0 : (int) (OlympiadConfig.OLYMPIAD_ELO_K_FACTOR * (1.0 - expectedOne))));
			playerTwoStat.set(Olympiad.ELO,
					eloTwo + (skipEloUpdate ? 0 : (int) (OlympiadConfig.OLYMPIAD_ELO_K_FACTOR * (0.0 - expectedTwo))));
			...
			try {
				// Save Fight Result
				saveResults(_playerOneID, _playerTwoID, _playerOneClass, _playerTwoClass, 1, _startTime, fightTime,
						(_type == CompetitionType.CLASSED ? 1 : 0));

				final Item item = _playerOne.getInventory().addItem(ItemProcessType.REWARD,
						OlympiadConfig.OLYMPIAD_BATTLE_REWARD_ITEM, finalReward, _playerOne, null);
			...
			playerOneStat.set(COMP_LOST, playerOneLost + 1);
			playerOneStat.set(Olympiad.ELO,
					eloOne + (skipEloUpdate ? 0 : (int) (OlympiadConfig.OLYMPIAD_ELO_K_FACTOR * (0.0 - expectedOne))));
			playerTwoStat.set(Olympiad.ELO,
					eloTwo + (skipEloUpdate ? 0 : (int) (OlympiadConfig.OLYMPIAD_ELO_K_FACTOR * (1.0 - expectedTwo))));
			...
			try {
				// Save Fight Result
				saveResults(_playerOneID, _playerTwoID, _playerOneClass, _playerTwoClass, 2, _startTime, fightTime,
						(_type == CompetitionType.CLASSED ? 1 : 0));

				final Item item = _playerTwo.getInventory().addItem(ItemProcessType.REWARD,
						OlympiadConfig.OLYMPIAD_BATTLE_REWARD_ITEM, finalReward, _playerTwo, null);
				final InventoryUpdate iu = new InventoryUpdate();
				iu.addModifiedItem(item);
			...
			// Save Fight Result
			saveResults(_playerOneID, _playerTwoID, _playerOneClass, _playerTwoClass, 0, _startTime, fightTime,
					(_type == CompetitionType.CLASSED ? 1 : 0));
			...
			playerTwoStat.set(COMP_DRAWN, playerTwoDrawn + 1);
			playerOneStat.set(Olympiad.ELO,
					eloOne + (skipEloUpdate ? 0 : (int) (OlympiadConfig.OLYMPIAD_ELO_K_FACTOR * (0.5 - expectedOne))));
			playerTwoStat.set(Olympiad.ELO,
					eloTwo + (skipEloUpdate ? 0 : (int) (OlympiadConfig.OLYMPIAD_ELO_K_FACTOR * (0.5 - expectedTwo))));
	...
	protected boolean makeCompetitionStart() {
		...
		// Advanced Elo: Featured Match Announcement
		final int elo1 = Olympiad.getNobleStats(_playerOneID).getInt(Olympiad.ELO);
		final int elo2 = Olympiad.getNobleStats(_playerTwoID).getInt(Olympiad.ELO);
		if (((elo1 + elo2) / 2) >= OlympiadConfig.OLYMPIAD_ELO_ANNOUNCE_THRESHOLD) {
			Broadcast.toAllOnlinePlayers("Olympiad: Featured Match in Arena " + (_stadiumID + 1) + "! "
					+ _playerOneName + " (" + elo1 + ") vs " + _playerTwoName + " (" + elo2 + ")");
		}
		try {
			for (Player player : _players) {
	...
	protected void announceGame() {
		...
		for (Spawn spawn : SpawnTable.getInstance().getSpawns(OLY_MANAGER)) {
			...
			if (manager != null) {
				manager.broadcastPacket(new CreatureSay(manager, ChatType.NPC_SHOUT, manager.getName(),
						"Olympiad " + gameType + " is going to begin in Arena " + (_stadiumID + 1) + " in a moment."));
			}
		}
	}

	protected void saveResults(int playerOne, int playerTwo, int playerOneClass, int playerTwoClass, int winner,
			long startTime, long fightTime, int classed) {
