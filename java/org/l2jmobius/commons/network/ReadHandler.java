 ...
 * This class implements {@link CompletionHandler} to process the data read from
 * the client, converting it into packets and executing them.
 * 
...
public class ReadHandler<T extends Client<Connection<T>>> implements CompletionHandler<Integer, T> {
	...
	private void handlePayload(T client) {
		...
		if (client.isRateLimited(buffer.remaining())) {
			client.disconnect();
			return;
		}
