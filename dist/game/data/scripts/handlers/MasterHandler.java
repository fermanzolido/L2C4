...
import handlers.admincommandhandlers.AdminDonation;
...
import handlers.bypasshandlers.MenuBypass;
...
import handlers.voicedcommandhandlers.Menu;
...
public class MasterHandler {
	...
	private static final Class<?>[][] HANDLERS = {
			...
			{
					// Voiced command registration (no per-command config).
					// CastleHandler.class,
					// ClanHandler.class,
					Menu.class,
					...
					MultilingualSupportConfig.MULTILANG_ENABLE && MultilingualSupportConfig.MULTILANG_VOICED_ALLOW
							? Lang.class
							: null,
					OfflinePlayConfig.ENABLE_OFFLINE_PLAY_COMMAND ? OfflinePlay.class : null,
					OfflineTradeConfig.ENABLE_OFFLINE_COMMAND
							&& (OfflineTradeConfig.OFFLINE_TRADE_ENABLE || OfflineTradeConfig.OFFLINE_CRAFT_ENABLE)
									? Offline.class
									: null,
	...
	public static void main(String[] args) {
		...
		registerHandlerMethods.entrySet().stream().filter(e -> e.getValue() == null)
				.forEach(e -> LOGGER.log(Level.WARNING, "Failed loading handlers of: "
						+ e.getKey().getClass().getSimpleName() + " seems registerHandler function does not exist."));
		...
		for (IHandler<?, ?> loadInstance : LOAD_INSTANCES) {
			LOGGER.log(Level.INFO,
					loadInstance.getClass().getSimpleName() + ": Loaded " + loadInstance.size() + " handlers.");
		}
