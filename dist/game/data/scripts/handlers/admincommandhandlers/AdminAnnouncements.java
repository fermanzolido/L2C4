		...
		switch (cmd) {
			...
			case "admin_announces": {
				...
				switch (subCmd) {
					case "add": {
						if (!st.hasMoreTokens()) {
							final String content = HtmCache.getInstance().getHtm(activeChar,
									"data/html/admin/announces/announces-add.htm");
						...
						if ((delay < (10 * 1000)) && ((type == AnnouncementType.AUTO_NORMAL)
						...
						if ((type == AnnouncementType.AUTO_CRITICAL) || (type == AnnouncementType.AUTO_NORMAL)) {
							announce = new AutoAnnouncement(type, content, activeChar.getName(), initDelay, delay,
									repeat);
						} else {
					...
					case "edit": {
						...
						if (!st.hasMoreTokens()) {
							String content = HtmCache.getInstance().getHtm(activeChar,
									"data/html/admin/announces/announces-edit.htm");
						...
						switch (announce.getType()) {
							...
							case EVENT: {
								switch (type) {
									...
									default: {
										activeChar.sendSysMessage(
												"Announce type can be changed only to NORMAL, CRITICAL or EVENT!");
										return false;
									}
								}
								break;
						...
						if ((delay < 10) && ((type == AnnouncementType.AUTO_NORMAL)
								|| (type == AnnouncementType.AUTO_CRITICAL))) {
							activeChar.sendSysMessage("Delay cannot be less then 10 seconds!");
							break;
						}
					...
					case "show": {
						...
						if (announce != null) {
							String content = HtmCache.getInstance().getHtm(activeChar,
									"data/html/admin/announces/announces-show.htm");
							final String announcementId = Integer.toString(announce.getId());
							final String announcementType = announce.getType().name();
							String announcementInital = "0";
							String announcementDelay = "0";
					...
					case "list": {
						...
						String content = HtmCache.getInstance().getHtm(activeChar,
								"data/html/admin/announces/announces-list.htm");
						final PageResult result = PageBuilder
								.newBuilder(AnnouncementsTable.getInstance().getAllAnnouncements(), 10,
										"bypass admin_announces list")
								.currentPage(page).pageHandler(NextPrevPageHandler.INSTANCE)
								.formatter(BypassParserFormatter.INSTANCE).style(ButtonsStyle.INSTANCE)
								.bodyHandler((pages, announcement, sb) -> {
									...
									if ((announcement.getType() == AnnouncementType.AUTO_NORMAL)
										...
										sb.append("<td width=60><button action=\"bypass admin_announces restart "
												+ announcement.getId()
												+ "\" value=\"Restart\" width=\"60\" height=\"21\" back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
									} else {
										sb.append(
												"<td width=60><button action=\"\" value=\"\" width=\"60\" height=\"21\" back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
									}

									if (announcement.getType() == AnnouncementType.EVENT) {
										sb.append("<td width=60><button action=\"bypass admin_announces show "
												+ announcement.getId()
												+ "\" value=\"Show\" width=\"60\" height=\"21\" back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
										sb.append("<td width=60></td>");
									} else {
										sb.append("<td width=60><button action=\"bypass admin_announces show "
												+ announcement.getId()
												+ "\" value=\"Show\" width=\"60\" height=\"21\" back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
										sb.append("<td width=60><button action=\"bypass admin_announces edit "
												+ announcement.getId()
												+ "\" value=\"Edit\" width=\"60\" height=\"21\" back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
									}
									sb.append("<td width=60><button action=\"bypass admin_announces remove "
											+ announcement.getId()
											+ "\" value=\"Remove\" width=\"60\" height=\"21\" back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
									sb.append("<td width=5></td>");
									sb.append("</tr>");
								}).build();
