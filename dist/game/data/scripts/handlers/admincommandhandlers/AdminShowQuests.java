...
/**
 * Admin quest display handler (needs rework).
 * 
...
public class AdminShowQuests implements IAdminCommandHandler {
	...
	private void showFirstQuestMenu(Player target, Player actor) {
		final StringBuilder replyMSG = new StringBuilder(
				"<html><body><table width=270><tr><td width=45><button value=\"Main\" action=\"bypass admin_admin\" width=45 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td><td width=180><center>Player: "
						+ target.getName()
						+ "</center></td><td width=45><button value=\"Back\" action=\"bypass admin_admin6\" width=45 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td></tr></table>");
		...
		replyMSG.append("<table width=250><tr><td><button value=\"CREATED\" action=\"bypass -h admin_charquestmenu "
				+ target.getName() + " 0\" width=95 height=21 back=\"bigbutton_over\" fore=\"bigbutton\"></td></tr>");
		replyMSG.append("<tr><td><button value=\"STARTED\" action=\"bypass -h admin_charquestmenu " + target.getName()
				+ " 1\" width=95 height=21 back=\"bigbutton_over\" fore=\"bigbutton\"></td></tr>");
		replyMSG.append("<tr><td><button value=\"COMPLETED\" action=\"bypass -h admin_charquestmenu " + target.getName()
				+ " 2\" width=95 height=21 back=\"bigbutton_over\" fore=\"bigbutton\"></td></tr>");
		replyMSG.append("<tr><td><br><button value=\"All\" action=\"bypass -h admin_charquestmenu " + target.getName()
				+ " 3\" width=95 height=21 back=\"bigbutton_over\" fore=\"bigbutton\"></td></tr>");
		replyMSG.append("<tr><td><br><br>Manual Edit by Quest number:<br></td></tr>");
		replyMSG.append(
				"<tr><td><edit var=\"qn\" width=65 height=12><br><button value=\"Edit\" action=\"bypass -h admin_charquestmenu "
						+ target.getName()
						+ " $qn custom\" width=65 height=21 back=\"L2UI_ch3.smallbutton2_over\" fore=\"L2UI_ch3.smallbutton2\"></td></tr>");
	...
	private void showQuestMenu(Player target, Player actor, String[] val) {
		try (Connection con = DatabaseFactory.getConnection()) {
			...
			switch (val[0]) {
				case "full": {
					replyMSG.append("<table width=250><tr><td>Full Quest List for <font color=\"LEVEL\">"
							+ target.getName() + "</font> (ID:" + ID + ")</td></tr>");
				...
				case "name": {
					...
					replyMSG.append(
							"<center><table width=250><tr><td width=70>Var</td><td width=40>Value</td><td>New Value</td><td>&nbsp;</td></tr>");
					req = con.prepareStatement("SELECT var,value FROM character_quests WHERE charId='" + ID
							+ "' and name='" + val[1] + "'");
					req.execute();
					rs = req.getResultSet();
					while (rs.next()) {
						...
						replyMSG.append("<tr><td>" + var_name + "</td><td>" + rs.getString(2)
								...
								+ "\" width=30 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td><td><button value=\"Del\" action=\"bypass -h admin_setcharquest "
								+ target.getName() + " " + val[1] + " " + var_name
								+ " delete\" width=30 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td></tr>");
					}
					...
					replyMSG.append("<tr><td><button value=\"Quest Complete\" action=\"bypass -h admin_setcharquest "
							...
							+ " state COMPLETED 1\" width=95 height=21 back=\"bigbutton_over\" fore=\"bigbutton\"></td>");
					replyMSG.append("<td><button value=\"Quest Complete\" action=\"bypass -h admin_setcharquest "
							+ target.getName() + " " + val[1]
							+ " state COMPLETED 0\" width=95 height=21 back=\"bigbutton_over\" fore=\"bigbutton\"></td></tr>");
					replyMSG.append(
							"</table><br><br><font color=\"ff0000\">Delete Quest from DB:</font><br><button value=\"Quest Delete\" action=\"bypass -h admin_setcharquest "
									+ target.getName() + " " + val[1]
									+ " state DELETE\" width=95 height=21 back=\"bigbutton_over\" fore=\"bigbutton\">");
					...
					if (exqdb) {
						if (exqch) {
							...
							replyMSG.append("<button value=\"Create Quest\" action=\"bypass -h admin_setcharquest "
									...
									+ " state CREATE\" width=95 height=21 back=\"bigbutton_over\" fore=\"bigbutton\"><br><br>");
							...
							replyMSG.append("<button value=\"Create & Complete\" action=\"bypass -h admin_setcharquest "
									+ target.getName() + " " + qnumber
									+ " state CC\" width=95 height=21 back=\"bigbutton_over\" fore=\"bigbutton\"><br><br>");
							replyMSG.append("</center></body></html>");
						}
					} else {
						replyMSG.append("<center><font color=\"ee0000\">Quest with number </font><font color=\"LEVEL\">"
								+ qnumber
