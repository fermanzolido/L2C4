...
public class CastleChamberlain extends Script {
	...
	private final String funcConfirmHtml(Player player, Npc npc, Castle castle, int func, int level) {
		if (isOwner(player, npc) && player.hasAccess(ClanAccess.CASTLE_MANAGE_FUNCTIONS)) {
			...
			if (level == 0) {
				...
				html.replace("%AgitDecoCost%", "<fstring p1=\"" + getFunctionFee(func, level) + "\" p2=\""
						+ (getFunctionRatio(func) / 86400000) + "\">6</fstring>");
				html.replace("%AgitDecoEffect%", "<fstring p1=\"" + level + "\">" + fstring + "</fstring>");
	...
	private void funcReplace(Castle castle, NpcHtmlMessage html, int func, String str) {
		...
		if (function == null) {
			...
			html.replace("%" + str + "Cost%", "<fstring p1=\"" + function.getLease() + "\" p2=\""
					+ (function.getRate() / 86400000) + "\">6</fstring>");
			html.replace("%" + str + "Expire%", "<fstring p1=\"" + calendar.get(Calendar.DATE) + "\" p2=\""
					+ (calendar.get(Calendar.MONTH) + 1) + "\" p3=\"" + calendar.get(Calendar.YEAR) + "\">5</fstring>");
			html.replace("%" + str + "Reset%",
	...
	public String onEvent(String event, Npc npc, Player player) {
		...
		switch (st.nextToken()) {
			...
			case "manage_tax": {
				if (isOwner(player, npc) && player.hasAccess(ClanAccess.CASTLE_VAULT)) {
					if (castle.getSiege().isInProgress()) {
						...
						html.replace("%next_tax_rate%", "0"); // Not available in C4.
			...
			case "manage_vault": {
				if (isOwner(player, npc) && player.hasAccess(ClanAccess.CASTLE_VAULT)) {
					...
					if (GeneralConfig.ALLOW_MANOR) {
						for (SeedProduction sp : CastleManorManager.getInstance()
					...
					html.replace("%tax_income_reserved%", "0"); // Not available in C4.
			...
			case "additional_functions": {
				htmltext = (isOwner(player, npc) && player.hasAccess(ClanAccess.CASTLE_MANAGE_FUNCTIONS))
			...
			case "goto": // goto listId locId
			{
				if (isOwner(player, npc) && player.hasAccess(ClanAccess.CASTLE_FUNCTIONS)) {
					...
					final int functionLevel = listIndex.length() == 2 ? Character.getNumericValue(listIndex.charAt(0))
							: -1;
					if (function.getLvl() == functionLevel) {
						final TeleportHolder holder = TeleporterData.getInstance().getHolder(npc.getId(),
								TeleportType.OTHER.name());
						if (holder != null) {
			...
			case "buffer": {
				if (isOwner(player, npc) && player.hasAccess(ClanAccess.CASTLE_FUNCTIONS)) {
					if (castle.getFunction(Castle.FUNC_SUPPORT) == null) {
						...
						final NpcHtmlMessage html = getHtmlPacket(player, npc,
			...
			// {
			// if (isOwner(player, npc) &&
			// player.hasClanPrivilege(ClanPrivilege.CS_MANAGE_SIEGE))
			// {
			// player.sendPacket(new ExShowDominionRegistry(castle.getResidenceId(),
			// player));
			// }
			// else
			// {
			...
			case "manor": {
				if (GeneralConfig.ALLOW_MANOR) {
					htmltext = (isOwner(player, npc) && player.hasAccess(ClanAccess.CASTLE_MANOR)) ? "manor.html"
							: "chamberlain-21.html";
				} else {
			...
			case "manors_cert": {
				if (npc.isMyLord(player)) {
					if (castle.getSiege().isInProgress()) {
						...
						if ((SevenSigns.getInstance().getPlayerCabal(player.getObjectId()) == SevenSigns.CABAL_DAWN)
								&& SevenSigns.getInstance().isCompetitionPeriod()) {
							final int ticketCount = castle.getTicketBuyCount();
							if (ticketCount < (FeatureConfig.SSQ_DAWN_TICKET_QUANTITY
									/ FeatureConfig.SSQ_DAWN_TICKET_BUNDLE)) {
								final NpcHtmlMessage html = getHtmlPacket(player, npc, "ssq_selldawnticket.html");
								html.replace("%DawnTicketLeft%", String.valueOf(FeatureConfig.SSQ_DAWN_TICKET_QUANTITY
										- (ticketCount * FeatureConfig.SSQ_DAWN_TICKET_BUNDLE)));
								html.replace("%DawnTicketBundle%",
										String.valueOf(FeatureConfig.SSQ_DAWN_TICKET_BUNDLE));
								html.replace("%DawnTicketPrice%", String.valueOf(
										FeatureConfig.SSQ_DAWN_TICKET_PRICE * FeatureConfig.SSQ_DAWN_TICKET_BUNDLE));
								player.sendPacket(html);
							} else {
				...
				if (npc.isMyLord(player)) {
					if (castle.getSiege().isInProgress()) {
						...
						if ((SevenSigns.getInstance().getPlayerCabal(player.getObjectId()) == SevenSigns.CABAL_DAWN)
							...
							if (ticketCount < (FeatureConfig.SSQ_DAWN_TICKET_QUANTITY
									/ FeatureConfig.SSQ_DAWN_TICKET_BUNDLE)) {
								final int totalCost = FeatureConfig.SSQ_DAWN_TICKET_PRICE
										* FeatureConfig.SSQ_DAWN_TICKET_BUNDLE;
								if (player.getAdena() >= totalCost) {
									takeItems(player, Inventory.ADENA_ID, totalCost);
									giveItems(player, FeatureConfig.SSQ_MANORS_AGREEMENT_ID,
											FeatureConfig.SSQ_DAWN_TICKET_BUNDLE);
	...
	public void onNpcManorBypass(OnNpcManorBypass evt) {
		...
		if (isOwner(player, npc)) {
			...
			switch (evt.getRequest()) {
				...
				default: {
					LOGGER.warning(getClass().getSimpleName() + ": " + player + " send unknown request id "
							+ evt.getRequest() + "!");
				}
			}
		}
