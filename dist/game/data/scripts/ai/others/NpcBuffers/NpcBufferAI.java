...
public class NpcBufferAI implements Runnable {
	...
	@Override
	public void run() {
		if ((_npc == null) || !_npc.isSpawned() || _npc.isDecayed() || _npc.isDead() || (_skillData == null)
		...
		switch (_skillData.getAffectScope()) {
			case PARTY: {
				if (player.isInParty()) {
					for (Player member : player.getParty().getMembers()) {
						if (LocationUtil.checkIfInRange(skill.getAffectRange(), _npc, member, true)
								&& !member.isDead()) {
			...
			case RANGE: {
				for (Creature target : World.getInstance().getVisibleObjectsInRange(_npc, Creature.class,
						skill.getAffectRange())) {
	...
	/**
	 * Verifies if the character is an friend and can be affected by positive
	 * effect.
	 * 
	 * @param player the player
	 * @param target the target
	 * @return {@code true} if target can be affected by positive effect,
	 ...
	 * @param player the player
	 * @param target the target
	 * @return {@code true} if target can be affected by negative effect,
	 *         {@code false} otherwise
	 */
