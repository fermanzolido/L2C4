...
public class CastleTeleporter extends Script {
	...
	public String onEvent(String event, Npc npc, Player player) {
		...
		if (event.equalsIgnoreCase("TELEPORT_EXECUTE")) {
			...
			final NpcSay msg = new NpcSay(npc, ChatType.NPC_SHOUT, "The defenders of " + npc.getCastle().getName()
					+ " castle will be teleported to the inner castle.");
			npc.getCastle().oustAllPlayers();
			npc.setScriptValue(0);

			// Broadcast to all players in the same map region.
	...
	public String onFirstTalk(Npc npc, Player player) {
		final Siege siege = npc.getCastle().getSiege();
		return npc.isScriptValue(0)
				? (siege.isInProgress() && (siege.getControlTowerCount() == 0)) ? "teleporter-02.html"
						: "teleporter-01.html"
				: "teleporter-03.html";
	}
