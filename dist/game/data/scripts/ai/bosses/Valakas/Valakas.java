...
public class Valakas extends Script {
	...
	private static final SkillHolder[] VALAKAS_REGULAR_SKILLS = {
			...
			new SkillHolder(4689, 1), // Valakas Fear (level 1 only)
	};
	...
	private static final byte WAITING = 1; // Valakas is spawned and someone has entered, triggering a 30 minute window
	...
	private Valakas() {
		...
		if (status == DEAD) {
			...
			if (temp > 0) {
				...
				// The time has expired while the server was offline. Spawn valakas in his cave
				// as DORMANT.
		...
		if (npc != null) {
			...
			// Regeneration && inactivity task
				...
				if ((GrandBossManager.getInstance().getStatus(VALAKAS) == FIGHTING)
						&& ((_timeTracker + 900000) < System.currentTimeMillis())) {
			...
			// Spawn cinematic, regen_task and choose of skill.
				...
				for (Player insidePlayer : BOSS_ZONE.getPlayersInside()) {
					if (insidePlayer.isHero() && GrandBossConfig.VALAKAS_RECOGNIZE_HERO) {
						npc.broadcastSay(ChatType.NPC_SHOUT, insidePlayer.getName()
	...
	private void callSkillAI(Npc npc) {
		...
		// target.
		if ((_actualVictim == null) || _actualVictim.isDead() || !(npc.isInSurroundingRegion(_actualVictim))
				|| (getRandom(10) == 0)) {
		...
		if (LocationUtil.checkIfInRange((skill.getCastRange() < 600) ? 600 : skill.getCastRange(), npc, _actualVictim,
				true)) {
			npc.getAI().setIntention(Intention.IDLE);
